...
const crews = [
  {
    ...
    icon: Radar,
    color: 'from-swarm-400 to-swarm-600',
    badge: 'bg-swarm-500/10 text-swarm-400 border border-swarm-500/20',
    phase: 'RESEARCH',
  },
  {
    key: 'match',
    ...
    icon: Target,
    color: 'from-swarm-300 to-swarm-500',
    badge: 'bg-swarm-500/10 text-swarm-300 border border-swarm-500/20',
    phase: 'MATCH',
  },
  {
    key: 'build',
    ...
    icon: Code2,
    color: 'from-swarm-400 to-swarm-700',
    badge: 'bg-swarm-500/10 text-swarm-400 border border-swarm-500/20',
    phase: 'BUILD',
  },
  {
    ...
    icon: Rocket,
    color: 'from-terminal-green to-swarm-600',
    badge: 'bg-terminal-green/10 text-terminal-green border border-terminal-green/20',
    phase: 'DEPLOY',
  },
  {
    key: 'portfolio',
  ...
  },
  {
    ...
    icon: ShieldAlert,
    color: 'from-human-400 to-human-600',
    badge: 'bg-human-500/10 text-human-400 border border-human-500/20',
    phase: 'SECURITY',
  },
  {
    key: 'social',
    ...
    icon: MessageSquare,
    color: 'from-swarm-300 to-human-400',
    badge: 'bg-swarm-500/10 text-swarm-300 border border-swarm-500/20',
    phase: 'SOCIAL',
  },
]
...
const phases = [
  { name: 'RESEARCH', label: 'Discover', color: 'text-swarm-400' },
  { name: 'MATCH', label: 'Validate', color: 'text-swarm-300' },
  { name: 'BUILD', label: 'Engineer', color: 'text-swarm-400' },
  { name: 'DEPLOY', label: 'Ship', color: 'text-terminal-green' },
  { name: 'PORTFOLIO', label: 'Showcase', color: 'text-swarm-300' },
  { name: 'SECURITY', label: 'Harden', color: 'text-human-400' },
  { name: 'SOCIAL', label: 'Respond', color: 'text-swarm-300' },
]
...
export function Pipeline() {
  ...
  return (
    <section id="pipeline-detail" className="py-24 md:py-32 relative overflow-hidden">
      ...
      <div className="relative max-w-6xl mx-auto px-6">
        ...
        <div className="text-center mb-20">
          ...
          <h2 className="text-4xl md:text-5xl font-black tracking-tight mb-4">
            <span className="text-gradient-swarm">{totalAgents} agents</span>, {crews.length} crews
          </h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {crews.map((crew) => (
            <div
              key={crew.key}
              className="group relative p-6 rounded-xl border border-space-700/50 bg-space-900/60 backdrop-blur-sm hover:border-swarm-500/30 transition-all duration-300 hover:-translate-y-1"
            >
              ...
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 font-mono">{crew.key}_crew</p>
                </div>
              </div>
              ...
              {/* Agents in crew */}
              <ul className="space-y-1 border-t border-space-700/40 pt-3">
                {crew.agents.map((a) => (
                  <li key={a} className="flex items-center gap-2 text-[11px] font-mono text-slate-500">
                    <span className="w-1 h-1 rounded-full bg-swarm-500/60" />
                    {a}
                  </li>
                ))}
              </ul>
              ...
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-3/4 h-px bg-gradient-to-r from-transparent via-swarm-500/0 to-transparent group-hover:via-swarm-500/50 transition-all duration-500" />
            </div>
          ))}
        </div>

        {/* Pipeline connector */}
        <div className="mt-16 flex items-center justify-center gap-4 font-mono text-xs text-slate-600">
          <span>complaint heard</span>
          <div className="w-32 h-px pipeline-flow" />
          <span className="text-swarm-400 font-semibold">live product shipped</span>
        </div>
      </div>
    </section>
  )
