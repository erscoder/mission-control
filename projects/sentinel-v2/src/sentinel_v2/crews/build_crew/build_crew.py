...
def build_crew(cycle: int = 1) -> Crew:
    ...
    backend_task = Task(
        description=(
            ...
            "- Do NOT create a Dockerfile, fly.toml, or `backend/package.json`. The flow has "
            "  already pre-baked the canonical NestJS pinned-version package.json, the "
            "  Dockerfile, and fly.toml in `{workspace_dir}/backend/` before you start. "
            "  Rewriting any of these is the leading cause of ERESOLVE peer-dep failures and "
            "  will be overwritten at deploy time anyway.\n"
