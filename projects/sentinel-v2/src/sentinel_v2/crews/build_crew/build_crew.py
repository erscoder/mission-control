...
def build_crew(cycle: int = 1) -> Crew:
    ...
    frontend_lead = Agent(
        ...
        backstory=(
            ...
            "PATCH MODE (RETRY ATTEMPTS) - HARD RULE: When the task input "
            "`existing_files` is non-empty, the workspace already contains code "
            "from a previous attempt and the build verification gate caught a "
            "specific failure (recorded in `revision_notes`). You MUST: "
            "(1) READ the failure stderr in revision_notes to identify the "
            "    SPECIFIC files that need fixing (TS2307 names a missing "
            "    module, parallel-routes names two files at the same path, "
            "    type errors name a file:line pair); "
            "(2) USE `list_files` and read existing files via list_files to confirm "
            "    the current shape before editing; "
            "(3) `write_file` ONLY for paths named in the failure stderr OR "
            "    new files required to satisfy a missing import that the "
            "    failure references. DO NOT rewrite untouched pages, "
            "    components, or styles. DO NOT regenerate the workspace; the "
            "    prior attempt's code is on disk and the canonical UI "
            "    primitives (Button, Card, Input, Label, lib/utils) are "
            "    pre-baked - re-writing them in their canonical paths is a "
            "    no-op but writing to a DIFFERENT path is forbidden; "
            "(4) DO NOT create alternate route paths (`(auth)/login/page.tsx` "
            "    when `login/page.tsx` already exists in `existing_files` is a "
            "    Next.js parallel-routes error - delete the duplicate or do "
            "    not create it); "
            "(5) every `write_file` call you make should be JUSTIFIABLE by a "
            "    line in the failure stderr. If you cannot point to a stderr "
            "    line that requires it, do not write the file.\n\n"
    ...
    backend_lead = Agent(
        ...
        backstory=(
            ...
            "PATCH MODE (RETRY ATTEMPTS) - HARD RULE: When the task input "
            "`existing_files` is non-empty, the workspace already contains code "
            "from a previous attempt and post-build verification caught a "
            "specific failure (recorded in `revision_notes`). You MUST: "
            "(1) READ revision_notes stderr to identify the SPECIFIC files "
            "    that need fixing (TS2307 names a missing module, TS2305 a "
            "    missing export, TS2339 a missing property, file:line pairs "
            "    point at the broken site); "
            "(2) READ existing files via `list_files` + `cat` BEFORE editing "
            "    so you do not overwrite the prior attempt's working code; "
            "(3) `write_file` ONLY for paths named in revision_notes OR new "
            "    files required to satisfy a missing import that revision_notes "
            "    references. DO NOT rewrite untouched services, controllers, "
            "    or DTOs. The canonical NestJS template files (PrismaService, "
            "    PrismaModule, StripeWebhookController, stripe.config.ts, "
            "    package.json, tsconfig*, fly.toml, Dockerfile, nest-cli.json) "
            "    are pre-baked and re-baked after every retry - never "
            "    overwrite them; "
            "(4) every `write_file` call you make should be JUSTIFIABLE by a "
            "    line in revision_notes stderr. If you cannot point to a "
            "    stderr line that requires the file change, do not write it.\n"
    ...
    _shared_inputs_block = (
        ...
        "If revision_notes is non-empty, this is a RETRY. existing_files is a "
        "dict[path -> sha1] listing every file already on disk. The leads MUST "
        "treat every path in existing_files as on-disk and ONLY write_file "
        "to paths explicitly named in revision_notes (the build stderr) OR "
        "new files required to satisfy a missing import. NEVER write_file to "
        "a path in existing_files for a different concern; doing so deletes "
        "the prior attempt's work and resets convergence. If you need to "
        "confirm content of an existing file, use list_files + read it first. "
        "Goal: surgical patch, not rewrite.\n"
    )
    ...
    frontend_task = Task(
        description=(
            ...
            "RETRY CONTEXT (read this BEFORE step 0):\n"
            "  revision_notes  = {revision_notes}\n"
            "  existing_files  = {existing_files}\n"
            "If revision_notes is non-empty AND existing_files is non-empty, this is a "
            "RETRY. Follow the PATCH MODE rule from your backstory: read the failure "
            "stderr, identify ONLY the files named in it, read existing files via "
            "list_files before editing, write_file ONLY for files explicitly required by "
            "the failure. Do NOT regenerate the whole tree — the prior attempt's code is "
            "on disk in the paths listed in existing_files, the canonical pre-baked "
            "primitives (Button, Card, Input, Label, lib/utils) are already present, "
            "skip steps that produce content already on disk.\n\n"
            "STEP 0 — DESIGN SYSTEM (do this FIRST on attempt 1; SKIP if "
            "`frontend/DESIGN.md` already appears in existing_files):\n"
    ...
    backend_task = Task(
        description=(
            ...
            "using `write_file`.\n\n"
            "RETRY CONTEXT (read this BEFORE Stripe step):\n"
            "  revision_notes  = {revision_notes}\n"
            "  existing_files  = {existing_files}\n"
            "If revision_notes is non-empty AND existing_files is non-empty, this is a "
            "RETRY. Follow the PATCH MODE rule from your backstory: read the failure "
            "stderr (TS2305/TS2307/TS2339 etc.), identify ONLY the files named in it, "
            "read existing files via list_files before editing, write_file ONLY for "
            "files explicitly required by the failure. Do NOT regenerate the whole "
            "src/ tree — the prior attempt's modules / services / DTOs are on disk in "
            "the paths listed in existing_files. The canonical pre-baked files "
            "(`src/prisma/prisma.{module,service}.ts`, `src/config/stripe.config.ts`, "
            "`src/modules/stripe/stripe.controller.ts`) are already present and re-baked "
            "after every retry, do NOT overwrite them.\n\n"
            "STRIPE PRODUCT CREATION (skip on retry if `STRIPE_PRODUCT_ID` already in "
            "the environment from a prior attempt; do this FIRST otherwise):\n"
