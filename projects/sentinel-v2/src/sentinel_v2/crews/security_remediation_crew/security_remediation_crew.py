...
def security_remediation_crew(cycle: int = 1) -> Crew:
    ...
    developer = Agent(
        ...
        backstory=(
            ...
            "1. Update dependencies one-by-one with `npm install <pkg>@<version> --save` (or "
            "   `--save-dev`) so npm rewrites only that one entry and re-resolves peers. Reach "
            "   for `write_file` on `package.json` only when npm install cannot express the "
            "   change, AND never touch the `@nestjs/*` version matrix — those are pinned by "
            "   Sentinel for cross-package compatibility. For Python use "
            "   `requirements.txt`/`pyproject.toml`, for Rust `Cargo.toml`.\n"
    ...
    apply_task = Task(
        description=(
            ...
            "  1. Update the dependency in-place WITHOUT rewriting the whole `package.json`. "
            "     Prefer `run_shell('npm install <pkg>@<version> --save', subdir='backend')` "
            "     (or 'frontend'). Only fall back to `write_file` on a non-`@nestjs/*` package "
            "     when npm install cannot express the change. NEVER rewrite the `@nestjs/*` "
            "     version matrix — Sentinel pins those for cross-package compatibility, and "
            "     touching one without the others guarantees an ERESOLVE peer-dep deadlock.\n"
            "  2. Run `npm install` in the right subdir via `run_shell` to confirm the lockfile "
            "     resolves.\n"
