...
def deploy_crew(cycle: int = 1) -> Crew:
    ...
    deployer = Agent(
        ...
        backstory=(
            ...
            "     using `run_shell` in `<workspace_dir>/backend/` against the literal "
            "     `database_url` task input. Do NOT use any other DATABASE_URL value: "
            "     Sentinel attached this app to the shared Fly Postgres cluster before "
            "     calling you, and `database_url` IS the connection string for the "
            "     dedicated database created for this app inside that cluster.\n"
            ...
            "     ...other non-secret env vars}, stage=True)`. ALL values above are "
            "     pre-provisioned by Sentinel and passed to you as literal task inputs; "
            "     substitute them verbatim, do NOT generate placeholder values, do NOT "
            "     call any stripe_* / fly postgres API yourself, do NOT skip any of the "
            "     four STRIPE_* keys. They MUST be staged in this same release so the "
            "     first boot of the app does not crash on the canonical "
            "     `stripe.config.ts` env validation. If `database_url` is the empty "
            "     string the app was already attached on a prior cycle and DATABASE_URL "
            "     is already a Fly secret on the app; SKIP staging DATABASE_URL but "
            "     still stage the four STRIPE_* keys.\n"
    ...
    deploy_task = Task(
        description=(
            ...
            "  database_url    = {database_url}   # empty string -> already attached\n\n"
