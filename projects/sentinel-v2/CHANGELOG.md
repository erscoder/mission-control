...
## 2026-05-04 · da0e0b9 - Disable declaration emit in canonical NestJS tsconfig

ComplianceDesk re-build attempt 4/6 failed with cascading TS4073 `Parameter X of public method from exported class has or is using private name Y` on `CreateReminderDto`, `ReminderFiltersDto`, and every other DTO referenced by a controller's public route handlers. The DTOs are correctly exported (`export class CreateReminderDto`, imported by direct path) so the agent could not figure out what to fix and burned every retry on the same error. Cause: canonical `tsconfig.json` set `"declaration": true`, which makes TypeScript emit `.d.ts` files for the build output. Declaration emit applies a stricter visibility check on every public method signature; for NestJS module-internal DTOs that aren't part of the package's public surface, TS4073 fires by design even when the class IS exported. A NestJS application does not need declaration emit — the runtime reads `dist/main.js` and never opens `.d.ts`. Declaration is for libraries that publish to npm. Disabling it eliminates the entire TS4073 class for every DTO pattern the build crew generates.
...
## 2026-05-04 · ead53e1 - C+D - post-attempt diff verification + structured stderr classification

A+B alone (commit ea3b1e2) were not enough on the ComplianceDesk re-build: attempt 2 failed `Module not found '@supabase/...'` (frontend agent imported a package not in the canonical pin), attempt 3 failed again at frontend.build, attempts 4-6 likely to follow. The leads still did not act surgically because (1) they had no signal naming exactly which files to touch beyond the 1500-char raw stderr blob and (2) when they DID regress and delete files from a prior attempt, nothing in the loop noticed or asked them to re-author. Implements the deferred C+D half of the patch protocol design. Three helpers in `flows/sentinel_loop.py`. (C) `_diff_workspace_manifests(pre, post)` returns `{added, modified, deleted}` from two manifests; ignores the `__truncated__` sentinel; pure dict diff. (D) `_classify_build_stderr(stderr_tail)` parses raw stderr into `{failure_class, affected_files, missing_imports, missing_exports, parallel_route_paths, raw_tail}`; strips ANSI codes; first-match classification across `next_parallel_routes`, `ts_module_not_found` (TS2307), `ts_missing_export` (TS2305), `ts_property_missing` (TS2339), `ts_type_mismatch` (TS2322/2345/2724), `next_module_not_found`, `eresolve`, `generic`; lists capped at 20 entries; raw tail capped at 500 chars (down from 1500). (D) `_format_retry_feedback(attempt, first_failure, classified)` renders the classified dict as a compact bullet brief: failure class, affected files, missing imports/exports, parallel route paths, then the trimmed raw tail. Used to replace the prior 1500-char stderr blob in `revision_notes`. Wired into `run_build` retry loop: pre-kickoff manifest captured for diff, replaced raw-tail feedback with `_format_retry_feedback` output, and post-kickoff diff scans for "unjustified deletions" — files the lead removed that are NOT mentioned in the failure stderr. When found, appends an `AGENT REGRESSION DETECTED` warning to the next attempt's feedback listing the missing files so the lead re-authors them surgically; logs `agent_regression` event with the deletion list. Adds 13 unit tests across `TestStderrClassification` (8) and `TestWorkspaceManifestDiff` (4), plus 1 boundary test. All known failure shapes from the live ComplianceDesk run are covered: `date-fns`, `PrismaClient`, `STRIPE_SECRET_KEY ProcessEnv`, parallel login routes, `@supabase` missing, `ERESOLVE`. 131 unit tests pass.

## 2026-05-03 · ea3b1e2 - Structured patch protocol + existing_files manifest on retries

ComplianceDesk re-build (post-canonical-UI-primitives) attempt 1 produced a complete frontend including `Button.tsx`; attempt 2's frontend lead omitted `Button.tsx` entirely, every dashboard page failed `Module not found: '@/components/ui/Button'`, attempts 3-6 burned the budget regenerating `components/ui/*` and reintroducing the same parallel-routes (`(auth)/login/page.tsx` clashing with `login/page.tsx`) and missing-import errors. Root cause: `Process.sequential` runs each task once per kickoff; retries re-run the whole pipeline (planner + leads); `revision_notes` carried only ~1500 chars of stderr tail with no signal that the prior attempt's code is on disk and should be preserved. Leads regenerated from scratch each time and rotated which files they remembered. Two coupled changes implementing the A+B "structured patch protocol" design (C+D deferred). (1) `flows/sentinel_loop.py` adds `_workspace_file_manifest(workspace_dir, max_files=600)` that walks the workspace, skips runtime artefacts (`node_modules/`, `dist/`, `.next/`, `out/`, `.cache/`, `.git/`, `coverage/`, `*.tsbuildinfo`), and emits `{relative_path: sha1[:12]}`. On overflow returns the first `max_files` paths plus a sentinel `__truncated__` key so the agent knows the workspace is bigger than what it received. `run_build` computes the manifest before every `crew.kickoff` and injects it as the new `existing_files` task input. Empty on attempt 1 (workspace freshly pre-baked, no agent code yet); populated on every retry. (2) `crews/build_crew/build_crew.py` tightens the lead prompts: `_shared_inputs_block` lists `existing_files` and a hard rule to `write_file` ONLY for paths named in `revision_notes` OR new files required to satisfy a missing import (surgical patch, not rewrite); Frontend Lead backstory adds a PATCH MODE block with five numbered steps (read failure stderr, identify specific files, read existing files via `list_files`, `write_file` only for files named in stderr, every write JUSTIFIABLE by a stderr line) plus an explicit warning about Next.js parallel-routes; Backend Lead backstory mirrors the same PATCH MODE block adapted for TS2305/TS2307/TS2339 file:line failure shapes plus an explicit list of pre-baked canonical files that must never be overwritten; both Frontend and backend `Task` descriptions include a RETRY CONTEXT header with `{revision_notes}` and `{existing_files}` placeholders CrewAI substitutes at kickoff so the agent reads the manifest in the task description (not just the static backstory). Adds `TestWorkspaceFileManifest` (5 tests) covering missing workspace, sha1 short hash, exclusion of `node_modules` / `dist` / `.next` / `out` / `.cache` / `.git` / `coverage`, `.tsbuildinfo` skipping, truncation sentinel. 118 unit tests pass. C+D from the design (post-attempt diff verification, structured stderr classification) deferred to measure A+B alone first; if convergence improves on the next ComplianceDesk run, C+D may not be needed.

## 2026-05-03 · bd58b09 - Canonical frontend UI primitives + cn helper auto-baked

ComplianceDesk re-build cycle (post-baseline-restore) exhausted all 6 attempts on `Module not found: Can't resolve '@/components/ui/Button'` and identical errors for adjacent routes. Backend converged in attempts 4-5 (prepin + stripeConfig + Prisma types). Failure migrated to FRONTEND in attempts 5-6: build agent wrote `Button.tsx` in attempt 1 but lost it on later retries (agent regression: each retry partially regenerates `frontend/src/components/ui/` and skips files it had on prior attempts). Every dashboard page imports Button, so one missing primitive crashes the entire frontend build and the workspace never converges. Same defensive pattern as backend protected files (`PrismaService`, `StripeWebhookController`, `stripe.config.ts`): canonicalise the primitives the agent always imports but routinely forgets to write. `_prebake_frontend_files` already walks the template tree and overwrites the workspace before AND after the build crew runs (commit history tracks both `pre_build_rebake` and `post_build_frontend_rebake` log events), so dropping the canonical files in place auto-bakes them and reverts any agent edit. New canonical files under `data/deploy_templates/nextjs_frontend/src/`: `components/ui/Button.tsx` (PascalCase, primary/secondary/ghost/destructive variants, sm/md/lg sizes, Tailwind only, React.forwardRef, no external deps), `components/ui/Card.tsx` (Card + CardHeader + CardTitle + CardDescription + CardContent + CardFooter, mirrors the shadcn shape agents import out of habit), `components/ui/Input.tsx`, `components/ui/Label.tsx`, `lib/utils.ts` (the `cn()` classNames helper imported by every UI primitive and dozens of agent components — when it is missing the failure cascades through every file that calls it). All five files declare themselves `Sentinel V2 protected file. Do NOT modify` so a future agent that decides to rewrite them gets reverted on the very next `_prebake_frontend_files` call. The `@/lib/utils` import works without extra config because canonical `tsconfig.json -> compilerOptions.paths` already maps `@/*` to `./src/*`. Adds 1 unit test in `test_prebake_deploy_files.py` asserting the 5 files land at expected paths and expose the expected exports. 8/8 prebake tests pass. Lands for the next build cycle (operator already rebuilt the container image after this commit; daemon needs `docker compose up -d sentinel` to pick it up).

## 2026-05-03 · a3bf937 - Generate Prisma client before npm run build

ComplianceDesk re-build attempt 4/6 (after compliancedesk was re-queued for baseline test) failed `nest build` with `TS2305: Module '"@prisma/client"' has no exported member 'PrismaClient'` and `TS2339: Property '$connect' does not exist on type 'PrismaService'`. Cause: `_verify_npm_build` runs `npm install` then `npm run build` directly; the canonical NestJS package.json shipped a `prisma:generate` script but no `postinstall` hook, so the generated client at `node_modules/.prisma/client/` was never emitted. Without that directory, the published `@prisma/client` package re-exports nothing useful and tsc fails on every PrismaClient/PrismaService import. Two fixes for defense-in-depth. (1) `data/deploy_templates/node_nestjs/package.json` adds `"postinstall": "prisma generate --schema=prisma/schema.prisma || echo 'prisma generate skipped (schema missing or invalid)'"`. The `|| echo` keeps it non-fatal for the early-attempt case where the build agent runs `npm install` before writing the schema. (2) `flows/sentinel_loop.py _verify_npm_build` backend path: between install and build, if `backend/prisma/schema.prisma` exists, run `npx --no-install prisma generate --schema=prisma/schema.prisma` with a 180s cap. Failures are logged into `info[prisma_generate_*]` but do NOT short-circuit the build; tsc surfaces the underlying schema error if any. Lands for the next build cycle (current container image was built before this commit; agents on the running cycle will not benefit). 105/105 sentinel_loop + file_tool tests pass.

## 2026-05-03 · 4d4af70 - Reject shell redirects in run_shell argv

ComplianceDesk re-build observed live: backend agent invoked `npx tsc 2>&1` via `run_shell`. `subprocess.run` uses `shell=False`, so `shlex.split` preserved `2>&1` as a single literal token; tsc treated it as a path argument and emitted `error TS6231: Could not resolve the path '2>&1' with the extensions ...` forever. Agent read the misleading error, retried the same shell form, the build attempt burned its iteration budget on a non-issue. Existing operator rejection caught `&&`, `||`, `;`, `|` only. Extends `_SHELL_OPS` in `tools/file_tool.py` to also reject `>`, `>>`, `<`, `<<`, `2>&1`, `2>`, `1>`, `&>`, `1>&2`, `2>&-`. Error string tightened: "subprocess.run uses shell=False so redirects and pipes do not work; issue one run_shell call per command and read the captured output from the tool result". 5 new tests in `test_file_tool.py` cover stderr-merge, stdout/append/stdin redirects. 28/28 file_tool tests pass. Lands for next build cycle (current container image was built before this commit; no live restart needed).

## 2026-05-03 · 9e41e45 - Sandbox daemon in Docker with `~/Sentinel` bind-mount

The daemon was running on the bare mac (PID 15036) and several `subprocess.run` paths bypass the just-landed `RunShellTool` argv validator (commit 404e053): `_verify_npm_build` calls `npm install` / `npm run build` directly, `fly_tool._run_flyctl` invokes `flyctl` without workspace cwd binding, `osv_scanner_tool` accepts arbitrary absolute `scan_dir`, `shared_postgres._run_fly` provisions the Fly Postgres cluster. Any whitelist gap or validator bug re-exposes the operator's entire `/Users/kike/*` to LLM-driven shell. Three coupled changes move the pipeline into a closed Linux-namespace sandbox using the existing `sentinel` service in `docker-compose.yml`. (1) `docker-compose.yml` adds `${HOME}/Sentinel:/root/Sentinel` bind on the `sentinel` service (read-write) and the `flask` service (read-only, future-proofs UI rendering of workspace contents). Pins `SENTINEL_WORKSPACES_ROOT=/root/Sentinel` on both services so `WORKSPACES_ROOT` resolves to the bind target instead of container root home. Default bridge network keeps full egress for npm registry, MiniMax, Jina, Stripe, Fly, Cloudflare. No `--privileged`, no `network_mode: host`. `flows/sentinel_loop.py:29 _resolve_workspace_dir` already re-derives the workspace path from env on every call (per its own docstring) and disregards persisted `state.workspace_dir`, so host `~/Sentinel/draft_X` and container `/root/Sentinel/draft_X` round-trip naturally without daemon code changes. (2) `src/sentinel_v2/tools/osv_scanner_tool.py` defense-in-depth: reject `scan_dir` resolving outside `WORKSPACES_ROOT`. Even with the daemon containerized, an agent calling `osv_scan('/etc')` wanders info-disclosure paths inside the container; the new guard short-circuits before `subprocess.run`. Reuses `WORKSPACES_ROOT` already exported from `tools/file_tool.py:25`. Existing osv tests gain a `workspaces_root` fixture (monkeypatches the module-level constant to `tmp_path`) so they still pass; 4 new tests cover the guard (rejects `/etc`, rejects `..` traversal, accepts root itself, accepts subpath). (3) `README.md` adds a durable "Daemon execution policy" block: continuous daemon MUST run via `docker compose up -d sentinel`; bare-mac runs reserved for one-shot debugging only. Notes root-owned workspace files (cleanup via `sudo rm` or `docker exec`), `docker exec` reachability caveat (does not protect against attacker with mac shell), macOS bind perf escape hatch (`:delegated` consistency). Verified end-to-end on the running container: `docker exec sentinel ls /Users/kike/Documents` returns "No such file or directory"; `rm -rf /tmp/pwn` via RunShellTool returns "shell-error: outside workspace"; `osv_scan('/etc')` returns the structured error; bind is bidirectional between `~/Sentinel` and `/root/Sentinel`. 16/16 osv tests + 392/392 host unit suite green. Deferred (Option C): per-call ephemeral container per RunShellTool invocation. Daemon-in-container is sufficient for the stated risk model ("que no puedan liarla los agentes y borrar el mac").

## 2026-05-03 · 15a064c - Pre-pin @nestjs/schedule + date-fns in canonical NestJS package.json

ComplianceDesk live cycle attempt 4/6 failed `npm run build` with TS2307 on `@nestjs/schedule` (Cron / CronExpression in reminders.service.ts) and `date-fns` (subDays / addDays in reminders.service.ts and reports.service.ts). Backend agent imports both but never `npm install`s them on top of the pre-baked package.json, and the post-build verification gate rejects each retry with the same module-resolution error. Same failure shape as the next-auth pre-pin (556f493) for the frontend canonical. Pin `@nestjs/schedule@4.1.2` and `date-fns@3.6.0` directly in `data/deploy_templates/node_nestjs/package.json` so every backend boots with these available and the agent never has to remember to install them.

## 2026-05-03 · 404e053 - Wipe stale build artifacts + sandbox run_shell argv + ban manual-fix prose

ComplianceDesk cycle observed live: backend agent gave up after `npm run build` failed, emitted "Requires manual fix: Delete the stale dist/ directory" instead of patching code or cleaning artifacts. Two compounding causes: (a) `_verify_npm_build` did not clean stale `dist/tsconfig.tsbuildinfo` between retries, so TypeScript incremental compilation read the prior attempt's build state against the freshly-rewritten `src/` and emitted spurious errors that no source patch could ever fix, (b) the agent prompt did not forbid the "ask the operator to do shell work" failure mode, so when it hit the spurious errors it surrendered. Plus a third concern raised by the operator: the `run_shell` whitelist allows `rm` / `mv` / `cp` and stops shell chains, but `subprocess.run(cwd=workspace)` does NOT stop an absolute path argument like `rm -rf /Users/kike/Documents` because the path is interpreted by `rm`, not by the shell. Three fixes. (1) `flows/sentinel_loop.py _verify_npm_build` wipes `dist/`, `tsconfig.tsbuildinfo`, `tsconfig.build.tsbuildinfo`, `node_modules/.cache` (plus `.next`, `out` for frontend) before every `npm run build` so each attempt starts from a deterministic baseline. (2) `crews/build_crew/build_crew.py` Backend Lead backstory: explicit hard rule that the agent IS the operator. Output containing "requires manual fix", "delete the dist/ directory", "clear the cache" or any variant asking the human to do shell work is itself a build-failure signal. The post-build verification gate rejects such output as a failed attempt and starts a fresh retry. (3) `tools/file_tool.py` adds `_validate_destructive_argv(binary, argv, cwd, workspace)` that, for path-mutating binaries (`rm`, `mv`, `cp`, `chmod`, `touch`, `mkdir`), rejects every non-flag positional argument resolving outside the workspace: absolute paths (`/etc/passwd`), `..` traversal past workspace, `~` expansion to operator home. Also removes `sh` and `bash` from the whitelist because `sh -c "<arbitrary>"` re-introduces full shell expansion that bypasses argv validation; agents that need a shell pipeline must split into separate `run_shell` calls (already standard everywhere except the abandoned manual-fix path). 7 new tests in `test_file_tool.py` cover absolute-path rejection, `~` rejection, `..` traversal rejection, in-workspace allow-through (`rm -rf dist` still works for the legitimate cleanup case), and `sh` / `bash` whitelist removal. 101/101 file_tool + sentinel_loop tests pass.

## 2026-05-03 · b659ea5 - Defense-in-depth QA gate vs LLM go_no_go hallucination

Build QA Lead is an LLM and shipped a draft where it returned `go_no_go='GO'` / `build_status='clean'` while the prior Code Reviewer and Security Engineer tasks both reported open CRITICAL findings (frontend Stripe webhook with empty `STRIPE_WEBHOOK_SECRET` fallback, missing row-level authorization on documents/reports). The single-source QA gate trusted the QA Lead's verdict, so the bad draft promoted to approval. Three coupled fixes. (1) `flows/sentinel_loop.py` adds `_extract_review_critical(result)` that re-parses code-review and security-audit task outputs from `result.tasks_output`, counting CRITICALs across multiple shapes (severity counters: `critical_count`, `vulnerability_count_by_severity.critical`; findings arrays with `severity == 'critical'`; reviewer/auditor own `go_no_go != 'GO'` treated as one critical when no count present; substring fallback). The QA gate force-fails with `build_failed_qa_gate_critical_override` when either reviewer reports `critical>0`, regardless of QA Lead. Helper restricts `tasks_output` to `(list, tuple)` so loose `Mock()` test fixtures do not auto-iterate into a Mock attribute. (2) `crews/build_crew/build_crew.py` Frontend Lead backstory: hard rule banning `app/api/stripe/webhook/**`, `pages/api/stripe/webhook**`, `app/api/webhooks/**`, any frontend `stripe.webhooks.constructEvent` call, any frontend read of `STRIPE_WEBHOOK_SECRET` or `STRIPE_SECRET_KEY`. Webhook is owned solely by the canonical NestJS controller. (3) `crews/build_crew/build_crew.py` Code Reviewer task: explicit AUTOMATIC CRITICAL triggers for (a) any frontend Stripe webhook handler, (b) any env read with empty-string or null fallback for `STRIPE_*`, `JWT_*`, `NEXTAUTH_*`, `*_SECRET`, `*_KEY`, `*_TOKEN` (e.g. `process.env.STRIPE_WEBHOOK_SECRET ?? ''`, `process.env.JWT_SECRET || ''`), required pattern is throw-on-empty at module load or Zod env schema with `.min(1)`, (c) hardcoded Stripe live or test secret literals (`sk_live_*`, `sk_test_*`, `whsec_*`). Adds 6 unit tests covering the new helper and the override path on both code-review and security-audit critical signals. 392/392 unit tests pass.
...
## 2026-05-02 · eabb541 - Replace plan_task with 6-stage sequential planner chain

The monolithic `plan_task` asked `strategic_manager` for an 11-section shipping plan in one shot; frontend and backend leads then wrote code from scratch and consistently hallucinated imports, types, and entire modules. External research confirmed MiniMax-M2.7 scores about 78% on SWE-Bench Verified (comparable to Kimi K2.6 / Qwen 3.6 Plus), so the model is not the bottleneck; generate-from-scratch with a single broad plan is. Replaces the one-shot plan with six small sequential planning Tasks (`plan_architecture`, `plan_domain`, `plan_services`, `plan_api`, `plan_infra`, `plan_files`) all driven by `strategic_manager`. Each stage's `expected_output` pins exact section headings (e.g. `## File Tree`, ` ```prisma `, `## Endpoints`); downstream tasks grep them. No `output_pydantic` (MiniMax emits malformed JSON often enough to crash `crew.kickoff()`). Context wiring pruned per stage: backend reads the whole chain, frontend reads architecture + api + files only, code review and security audit get the relevant slices plus the leads' output. Tasks list grows 6 -> 11, agents stays at 6, process stays `Process.sequential`. Stale `test_build_crew_hierarchical_process` renamed to `test_build_crew_sequential_process`; stale `test_build_crew_has_manager_llm` deleted. Three new tests pin the 11-task layout, planner-chain ownership, and `plan_files in frontend_task.context`. 419 unit + integration tests green.
...
## 2026-05-02 · 805fb0c - Shared Fly Postgres cluster, per-app DB attach in run_deploy

All Sentinel-shipped apps now share ONE Fly Postgres cluster. Each app gets its own database (and Fly user) inside that cluster, exposed via a per-app `DATABASE_URL` Fly secret. One billing line for the fleet instead of one per app, simpler to scale storage and instance type once. Cluster is created lazily on the FIRST `run_deploy` entry: build verification has already passed AND the user has approved the deploy gate, so it is never spun up for a draft that does not ship. New `flows/shared_postgres.py` exposes `ensure_shared_pg_cluster` (state -> `fly postgres list` -> `fly postgres create`, idempotent across container restarts via `infra_state`) and `attach_db_for_app` (`fly postgres attach`, idempotent). All shell-outs use `subprocess.run`; nothing is exposed to the LLM. Defaults `sentinel-shared-pg` / `fra` / `shared-cpu-1x` / 10GB / single node, each overrideable via env. `run_deploy` calls these between pre-bake and `_provision_stripe_resources` so DATABASE_URL + the four STRIPE_* secrets land in the same Fly release. Deploy crew prompt now references the literal `database_url` task input and explicitly skips staging it when the value is empty (already attached on a prior cycle). 12-case `test_shared_postgres.py` covers state shortcut, fly-list recovery, create path, already-taken race, genuine failure, attach success / already-attached / failure, plus URL parsing parametrize. 382 unit tests green.

## 2026-05-02 · 1d9d418 - Add infra_state key/value table

Generic key/value store for infrastructure that lives outside any single draft. First user is the shared Fly Postgres cluster name + region, persisted on first successful deploy so subsequent deploys reuse the cluster. Schema added to `db._SCHEMA`; `get_infra` / `set_infra` / `delete_infra` helpers in `db.py`. 6 unit tests cover upsert semantics + isolation from the drafts table.
...
Fix: split `_provision_stripe_for_draft` into a Stripe-only helper `_provision_stripe_resources` that creates the Product + Webhook BEFORE the deploy crew kicks off (and outside the retry loop). The fresh signing secret + product/price IDs are passed to the crew as task inputs (`stripe_secret_key`, `stripe_webhook_secret`, `stripe_product_id`, `stripe_price_id`). The crew's step 3 `fly_secrets_set` call now stages the Stripe values alongside `DATABASE_URL`, so the first release boots with `stripe.config.ts` validation passing.

F-01 is closed at the same three layers: provisioning refuses on empty secret, daemon `STRIPE_SECRET_KEY` is required, and `stripe.config.ts` throws on missing env at module load. 335 unit tests green (was 336 pre-refactor; one Fly-side-effect test from the orchestration class was removed because the helper no longer touches Fly).

## 2026-05-01 · 0e9abbb - Dynamic per-app Stripe Product + Webhook provisioning (S1-S4)

Closes audit blocker F-01 (Stripe webhook empty-string fallback). Lifts Stripe orchestration out of agent prompts into Python code. New `_provision_stripe_for_draft` helper in `flows/sentinel_loop.py` (later replaced by `_provision_stripe_resources` in `1a2b006`), pure-function helpers `provision_product_for_draft` and `provision_webhook_for_draft` extracted from the BaseTool wrappers in `tools/stripe_tool.py`. `StripeCreateWebhookTool` changed from idempotency-by-reuse (returned `secret=None` on retries, the upstream cause of F-01) to idempotency-by-replace: any prior endpoint matching `metadata.sentinel_draft_id` or matching the URL is deleted, then a fresh one is created so the live signing secret is always captured. New protected NestJS templates `data/deploy_templates/node_nestjs/src/config/stripe.config.ts` (fail-fast on missing `STRIPE_SECRET_KEY`/`STRIPE_WEBHOOK_SECRET` at boot) and `src/modules/stripe/stripe.controller.ts` (no-fallback signature verification with `constructEvent`). `_prebake_deploy_files` refactored to walk template tree recursively. Build crew prompt forbids `|| ''` fallbacks. Deploy crew loses `stripe_create_webhook` tool. New `tests/unit/test_stripe_provisioning.py` with 11 tests; `test_stripe_tool.py` webhook tests rewritten for delete-and-recreate; 5 pre-existing `run_deploy` tests patched.
